#!/usr/bin/env python
"""
RBF interpolation with automatic parameter selection.
"""
...
class RBF:
    ...
    __slots__ = [
        ...
        '_crs', '_fold_spacing', 'n_jobs', 'best_params'
    ]
    
    def __init__(self, tgt_survey, reduction_method, x, y, z, grid_res=10.0, tgt_num_pts=5000, plot_outputs=False, n_jobs=-1):
            ...
            for smoothing in param_grid['smoothing']:
                for neighbors in param_grid['neighbors']:
                    ...
                    try:
                        ...
                        print(f"  {kernel:20s} smooth={smoothing:6.3f} neighbors={str(neighbors):5s}: "
                        ...
                        print(f"  {kernel:20s} smooth={smoothing:6.3f} neighbors={str(neighbors):5s}: FAILED ({e})")
                        continue
    ...
    def _interpolate(self, chunk_size=500, auto_tune=True):
        ...
        if self.best_params is not None:
            ...
            neighbors = None  # Use all points (RBFInterpolator handles this efficiently)
        
        print(f"Using RBF parameters: kernel={kernel}, smoothing={smoothing}, neighbors={neighbors}")
    ...
    def _write_output(self, output_path=None):
        ...
        if output_path is None:
            output_path = self.tgt_survey.parent / f"RBF_{self.reduction_method}_{self.tgt_num_pts}.tif"
        else:
        ...
        with rasterio.open(output_path, 'w', **self.meta) as dst:
            ...
            if self.cv_metadata:
                ...
                tags = {f'cv_{k}': str(v) for k, v in summary.items()
                        if isinstance(v, (int, float, str, type(None)))}
                tags['cv_results_json'] = json.dumps(summary, sort_keys=True)
                tags['cv_folds_json'] = json.dumps(self.cv_metadata.get('folds', []), sort_keys=True)
                dst.update_tags(**tags)
        
        return output_path
    
    def generate(self, output_path=None, auto_tune=True):
        """
